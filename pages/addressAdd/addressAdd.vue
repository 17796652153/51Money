...
<view class="bindingBox">
     ...
     <view class="bindingBoxInputView" @tap="showDistpickerFun">
          <view :class="'input ' + (!provinceName[provinceSelIndex] ? 'input-placeholder' : '')">
               <block v-if="provinceName[provinceSelIndex]">{{provinceName[provinceSelIndex]}}
                    {{cityName[citySelIndex]}} {{districtName[districtSelIndex]}}</block>
               <block v-else>请选择所在地区</block>
          </view>
  ...
  methods: {
    ...
    savePersonInfo() {
      ...
      if (uni.getStorageSync('openid') == '') {
        ...
        // 对接接口
        let url = 'api/users/addressInsert';
        let date = {
