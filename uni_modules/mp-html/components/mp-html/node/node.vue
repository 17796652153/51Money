<template>
  <view :id="attrs.id" :class="'_'+name+' '+attrs.class" :style="attrs.style">
    <block v-for="(n, i) in childs" v-bind:key="i">
      ...
      <text v-else-if="n.text" :user-select="n.us" decode selectable="true">{{n.text}}</text>
      <!-- #endif -->
      <text v-else-if="n.name==='br'" selectable="true">\n</text>
      <!-- 链接 -->
      ...
      <rich-text v-else-if="handler.use(n)" :id="n.attrs.id" :style="n.f" :nodes="[n]" selectable="true" />
      <!-- #endif -->
      <!-- #ifndef H5 || MP-WEIXIN || MP-QQ || APP-PLUS || MP-360 -->
      <rich-text v-else-if="!n.c" :id="n.attrs.id" :style="n.f+';display:inline'" :preview="false" :nodes="[n]" selectable="true" />
      <!-- #endif -->
      <!-- 继续递归 -->
