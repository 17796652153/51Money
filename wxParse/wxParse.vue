...
<template name="wxParse">
  ...
  <block v-if="item.node == 'element'">
    ...
    <block v-else-if="item.tag == 'img'">
      <template is="wxParseImg" :data="item" style="width：100%"></template> 
    </block>
...
<template name="wxParse0">
  ...
  <view :class="item.classStr + ' wxParse-' + item.tag" :style="item.styleStr">
    ...
    <!--img类型-->
    <block v-else-if="item.tag == 'img'">
      <block data-type="template" data-is="wxParseImg" data-attr="item" style="width：100%">
  <image :class="item.classStr + ' wxParse-' + item.tag" :data-from="item.from" :data-src="item.attr.src" :data-idx="item.imgIndex" :src="item.attr.src" @load="wxParseImgLoad" @tap="wxParseImgTap" mode="widthFix"></image>
</block> 
    </block>
...
function wxParse(bindName = 'wxParseData', type = 'html', data = '<div class="color:red;">数据不能为空</div>', target, imagePadding) {
  ...
  if (type == 'html') {
    transData = HtmlToJson.html2json(data, bindName); // console.log(JSON.stringify(transData, ' ', ' '));
  } else if (type == 'md' || type == 'markdown') {
    var converter = new showdown.Converter();
    var html = converter.makeHtml(data);
    transData = HtmlToJson.html2json(html, bindName); // console.log(JSON.stringify(transData, ' ', ' '));
  }
