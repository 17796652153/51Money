<template>
	<scroll-view class="time-line-wrap">
		<view class="time-line">
			<!-- 标题 -->
			<view class="time-line-title">{{ title }}</view>
			<!-- 时间轴 -->
						...
						<view class="line-info-content">
							...
							>
								...
								<view class="info-content">
									...
									<!-- 图片 -->
									<view><image v-if="item.img" :src="item.img" style="margin-top: 10rpx;" mode="aspectFit"></image></view>
								</view>
							</view>
						</view>
					</view>
...
export default {
	data() {
		return {
			// 数据
			time_line_list: this.timeList, // 使用获取到的传值
			HEIGHT: 0, //屏幕高度
			result: [], //监听的结果
			current: [], //当前在屏幕内的
			sum: 0, //加载完成个数
			...
			for (let i = 0; i < this.result.length; i++) {
				for (let j = 0; j < this.result[i].info.length; j++) {
					...
					if (this.current[j].is != 'ok') {
						// 是否进入视野
						if (this.current[j].tag == this.result[i].info[j].tag && this.result[i].info[j].domTop + 90 < this.HEIGHT) {
							// 加载动画
